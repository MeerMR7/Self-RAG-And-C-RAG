...
def web_search(state):
    ...
    try:
        ...
        web_docs = [Document(page_content=r["content"], metadata={"source": r["url"], "type": "web"}) for r in results if "content" in r]
        steps.append(f"🌐 Web search returned {len(web_docs)} results.")
        return {**state, "documents": state.get("documents", []) + web_docs, "workflow_steps": steps, "web_searched": True}
...
def generate(state):
    ...
    context = "\n\n".join([f"[Source {i+1}]: {doc.page_content}" for i, doc in enumerate(state["documents"])]) if state["documents"] else "No context found."
...
def check_hallucination(state):
    ...
    try:
        result = (hallucination_prompt | llm).invoke({"context": context[:1500], "answer": state["generation"][:500]})
        if "yes" in result.content.lower():
...
def build_graph():
    ...
    graph.add_conditional_edges("grade_documents", decide_after_grading, {"rewrite_query": "rewrite_query", "generate": "generate"})
...
def run_pipeline(question: str) -> dict:
    app = build_graph()
    result = app.invoke(RAGState(question=question, documents=[], generation="", workflow_steps=[], web_searched=False))
    return result
